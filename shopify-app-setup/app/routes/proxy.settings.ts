...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  try {
    ...
    // Try to fetch the JS from storage and pass it through with correct MIME type
    try {
      ...
      console.error('[Proxy Settings] Failed to fetch storage file:', err);
    }

    // Fallback to safe defaults (no DB)
    const fallback = `
// Default cart settings fallback (Remix proxy)
...
`;
    ...
    console.error('[Proxy Settings] Internal error:', error);
